...
use Foodchow\Includes\Library\TGMPA;
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function init_actions() {
			if ( apply_filters( $this->theme_name . '_enable_setup_wizard', true ) && current_user_can( 'manage_options' ) ) {
				...
				if ( class_exists( '\Foodchow\Includes\Library\TGMPA' ) && isset( $GLOBALS['tgmpa'] ) ) {
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard','foodchow' ), esc_html__( 'Setup Wizard' ,'foodchow'), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard' ,'foodchow'), esc_html__( 'Setup Wizard','foodchow' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction','foodchow' ),
...
//			);
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying','foodchow' ),
			) );
		...
		public function setup_wizard_header() {
			...
			// avoid theme check issues.
				echo '<t';
				echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard','foodchow' ) . '</ti' . 'tle>'; ?>
		...
		public function setup_wizard_footer() {
			...
			<?php if ( 'next_steps' === $this->step ) : ?>
				...
				href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard','foodchow' ); ?></a>
			<?php endif; ?>
		</body>
		<?php
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.','foodchow' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.','foodchow' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!' ,'foodchow'); ?></a>
						<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
							class="button button-large"><?php esc_html_e( 'Not right now','foodchow' ); ?></a>
						</p>
						<?php
					} else if ( get_option( 'envato_setup_complete', false ) ) {
						?>
						<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.','foodchow' ), wp_get_theme() ); ?></h1>
						<p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ','foodchow' ); ?></p>
						<ul>
							<li>
								<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
									class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again','foodchow' ); ?></a>
								</li>
								<li>
									<form method="post">
										<input type="hidden" name="reset-font-defaults" value="yes">
										<input type="submit" class="button-primary button button-large button-next"
										value="<?php esc_attr_e( 'Reset font style and colors','foodchow' ); ?>" name="save_step"/>
							...
							<p class="envato-setup-actions step">
								<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
									class="button button-large"><?php esc_html_e( 'Cancel' ,'foodchow'); ?></a>
								</p>
								<?php
							} else {
								?>
								<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.','foodchow' ), wp_get_theme() ); ?></h1>
								<p><?php printf( esc_html__( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.','foodchow' ), wp_get_theme() ); ?></p>
								<p><?php esc_html_e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!','foodchow' ); ?></p>
								<p class="envato-setup-actions step">
									<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
										class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!' ,'foodchow'); ?></a>
										<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.','foodchow' ); ?></strong>
				</p>
				<?php
				return true;
			}
		...
		public function envato_setup_download_media() {
			...
			?>
			<h1><?php esc_html_e( 'Default Content' ,'foodchow'); ?></h1>
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
				<p><?php esc_html_e( 'It looks like you already have media installed on this website. If you would like to install the default demo media as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.','foodchow' ); ?></p>
				<?php } else { ?>
				<p><?php printf( esc_html__( 'It\'s time to download some media for your new WordPress website. Choose media if you would like to download and click Continue. It is recommended to leave it selected. Once inserted, this media can be managed from the WordPress admin dashboard. ','foodchow' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
						<tr>
							<td class="check"></td>
							<th class="item"><?php esc_html_e( 'Item','foodchow' ); ?></th>
							<th class="description"><?php esc_html_e( 'Description','foodchow' ); ?></th>
							<th class="status"><?php esc_html_e( 'Status','foodchow' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<tr class="envato_default_content" data-content="download_media">
							...
							<td><label
								for="default_content_download_media"><?php esc_html_e( 'Download Media', 'foodchow' ); ?></label>
							</td>
							<td class="description"><?php esc_html_e( 'This will download default Media as seen in the demo.', 'foodchow' ); ?></td>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						...
						data-callback="download_media_data"><?php esc_html_e( 'Continue','foodchow' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-large button-next"><?php esc_html_e( 'Skip this step','foodchow' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
					</form>
					<?php
				...
				function ajax_media_data() {
					...
					if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
						wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found','foodchow' ) ) );
					}
					...
					if ( true ) {
				// install the content!
						$upload_dir = foodchow_set( wp_get_upload_dir(), 'basedir' );
						if ( ! is_dir( $upload_dir . '/foodchow' ) ) {
							$wp_filesystem->mkdir($upload_dir . '/foodchow');
						}
						$zipfile = $upload_dir . '/foodchow/media.zip';

						$this->log( ' -!! STARTING SECTION for ' . $_POST['content'] );
						set_time_limit(0);

						if ( file_exists( $zipfile ) ) {
							unzip_file( $zipfile, $upload_dir . '/foodchow' );
						} else {
							$file = wp_remote_get( 'https://s3.amazonaws.com/webinane-downloads/foodchow_media.zip', array('timeout' => 300 ) );
						...
						$json = array(
							'done'    => 1,
							'message' => esc_html__( "Success", 'foodchow' ),
							'debug'   => $result,
							'logs'    => $this->logs,
							'errors'  => $this->errors,
						);
				...
				wp_send_json( array(
					'error'   => 1,
					'message' => esc_html__( 'Error' ,'foodchow'),
					'logs'    => $this->logs,
					'errors'  => $this->errors,
				) );
			}
		...
		public function envato_setup_default_plugins() {
			...
			Foodchow\Includes\Library\tgmpa_load_bulk_installer();
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins','foodchow' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:' ,'foodchow'); ?></p>
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.','foodchow'  ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.','foodchow' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						class="button-primary button button-large button-next"
						data-callback="install_plugins"><?php esc_html_e( 'Continue','foodchow' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-large button-next"><?php esc_html_e( 'Skip this step','foodchow' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
				...
				public function ajax_plugins() {
					if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
						wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found' ,'foodchow') ) );
					}
					$json = array();
			...
			// what are we doing with this plugin?
					foreach ( $plugins['activate'] as $slug => $plugin ) {
						if ( $_POST['slug'] == $slug ) {
							$json = array(
								...
								'action2'       => - 1,
								'message'       => esc_html__( 'Activating Plugin','foodchow' ),
							);
							break;
						}
					}
					foreach ( $plugins['update'] as $slug => $plugin ) {
						if ( $_POST['slug'] == $slug ) {
							$json = array(
								...
								'action2'       => - 1,
								'message'       => esc_html__( 'Updating Plugin','foodchow' ),
							);
							break;
						}
					}
					foreach ( $plugins['install'] as $slug => $plugin ) {
						if ( $_POST['slug'] == $slug ) {
							$json = array(
								...
								'action2'       => - 1,
								'message'       => esc_html__( 'Installing Plugin','foodchow' ),
							);
							break;
						}
					}
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success','foodchow' ) ) );
			}
			exit;
		...
		private function _content_default_get() {
			...
			foreach ( $available_content as $post_type => $post_data ) {
				if ( count( $post_data ) ) {
					...
					$content[ $post_type ] = array(
						...
						'description'      => sprintf( esc_html__( 'This will create default %s as seen in the demo.','foodchow' ), $post_type_title ),
						'pending'          => esc_html__( 'Pending.' ,'foodchow'),
						'installing'       => esc_html__( 'Installing.' ,'foodchow'),
						'success'          => esc_html__( 'Success.' ,'foodchow'),
			...
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets','foodchow' ),
				'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.' ,'foodchow'),
				'pending'          => esc_html__( 'Pending.','foodchow' ),
				'installing'       => esc_html__( 'Installing Default Widgets.','foodchow' ),
				'success'          => esc_html__( 'Success.' ,'foodchow'),
			...
			$content['settings'] = array(
				'title'            => esc_html__( 'Settings' ,'foodchow'),
				'description'      => esc_html__( 'Configure default settings.' ,'foodchow'),
				'pending'          => esc_html__( 'Pending.','foodchow' ),
				'installing'       => esc_html__( 'Installing Default Settings.','foodchow' ),
				'success'          => esc_html__( 'Success.','foodchow' ),
		...
		/*	$content['layerslider'] = array(
				'title'            => esc_html__( 'LayerSlider','foodchow' ),
				'description'      => esc_html__( 'Install layerslider demo content.','foodchow' ),
				'pending'          => esc_html__( 'Pending.','foodchow' ),
				'installing'       => esc_html__( 'Installing layerslider content.','foodchow' ),
				'success'          => esc_html__( 'Success.' ,'foodchow'),
			...
			$content['revslider'] = array(
				'title'            => esc_html__( 'Revslider','foodchow' ),
				'description'      => esc_html__( 'Install revslider demo content.','foodchow' ),
				'pending'          => esc_html__( 'Pending.','foodchow' ),
				'installing'       => esc_html__( 'Installing revslider content.','foodchow' ),
				'success'          => esc_html__( 'Success.' ,'foodchow'),
		...
		public function envato_setup_default_content() {
			...
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
				<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.','foodchow' ); ?></p>
				<?php } else { ?>
				<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ','foodchow' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
						<tr>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						class="button-primary button button-large button-next"
						data-callback="install_content"><?php esc_html_e( 'Continue','foodchow' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-large button-next"><?php esc_html_e( 'Skip this step','foodchow' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
		...
		private function _process_post_data( $post_type, $post_data, $delayed = 0, $debug = false ) {
			...
			// contact form 7 id fixes.
				...
				if ( preg_match_all( '#\[static_block[^\]]*\]#', $content, $matches ) ) {
					foreach ( $matches[0] as $match_id => $string ) {
						if ( preg_match( '#id="(\d+)"#', $string, $id_match ) ) {
							$new_id = $this->_imported_post_id( $id_match[1] );
							if ( $new_id ) {
				...
				if ( preg_match_all( '#\[forms[^\]]*\]#', $content, $matches ) ) {
					foreach ( $matches[0] as $match_id => $string ) {
						if ( preg_match( '#id="(\d+)"#', $string, $id_match ) ) {
							$new_id = $this->_imported_post_id( $id_match[1] );
							if ( $new_id ) {
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( ! $upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond','foodchow' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size','foodchow' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded','foodchow' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s' ,'foodchow'), size_format( $max_size ) ) );
				}
			}
		...
		public function envato_setup_color_style() {
			...
			?>
			<h1><?php esc_html_e( 'Site Style','foodchow' ); ?></h1>
			<form method="post">
				<p><?php esc_html_e( 'Please choose your site style below.','foodchow' ); ?></p>
					...
					<ul>
						...
						foreach ( $this->site_styles as $style_name => $style_data ) {
							...
							if($style_name=='style1') { 
								echo esc_attr('Main Demo With ( VC )');
							}
							if($style_name=='style2') { 
								echo esc_attr('Main Demo With RTL'); 

							}
							if($style_name=='style3') { 
								echo esc_attr('Main Demo With ( Elementor )');
							}
							?></h3>
						</li>
						<?php } ?>
				...
				<p><em>
					...
					<?php printf( esc_html__( 'Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and Web Development knowledge. We recommend hiring an expert from%sEnvato Studio%s to assist with any advanced website changes.' ,'foodchow'), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>' ); ?>
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next"
					value="<?php esc_attr_e( 'Continue' ,'foodchow'); ?>" name="save_step"/>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						class="button button-large button-next"><?php esc_html_e( 'Skip this step','foodchow' ); ?></a>
						<?php wp_nonce_field( 'envato-setup' ); ?>
					</p>
		...
		public function envato_setup_logo_design() {
			...
			?>
			<h1><?php esc_html_e( 'Logo','foodchow' ); ?></h1>
			<form method="post">
				<p><?php printf( esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.' ,'foodchow'), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>' ); ?></p>
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo','foodchow' ); ?></a>
						</td>
					</tr>
				</table>
		...
		public function envato_setup_updates() {
			?>
			<h1><?php esc_html_e( 'Theme Updates','foodchow' ); ?></h1>
			...
			<form method="post">
				...
				if ( count( $my_items ) ) {
					...
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-large button-next button-primary"><?php esc_html_e( 'Continue','foodchow' ); ?></a>
						</p>
						<?php
					} else {
						?>
						<p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.','foodchow' ); ?></p>
						<p><?php esc_html_e( 'When an update becomes available it will show in the Dashboard with an option to install.','foodchow' ); ?></p>
						<p>
							<em>
								<?php esc_html_e(' On the next page you will be asked to Login with your ThemeForest account and grant permissions to enable Automatic Updates. If you have any questions please.','foodchow' ); ?> <a href="https://www.webinane.com/" target="_blank"><?php esc_html_e(' contact us','foodchow' ); ?></a>.</em>
							</p>
							<p class="envato-setup-actions step">
								<input type="submit" class="button-primary button button-large button-next"
								value="<?php esc_attr_e( 'Login with Envato' ,'foodchow'); ?>" name="save_step"/>
								<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
									class="button button-large button-next"><?php esc_html_e( 'Skip this step','foodchow' ); ?></a>
									<?php wp_nonce_field( 'envato-setup' ); ?>
								</p>
		...
		public function envato_setup_customize() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					class="button button-primary button-large button-next"><?php esc_html_e( 'Continue','foodchow' ); ?></a>
				</p>
			...
			public function envato_setup_help_support() {
				...
				<ul>
					...
					<li>Installation services (this is available through <a
						href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall"
						target="_blank"><?php esc_html_e( 'Envato Studio' ,'foodchow'); ?> </a>)
					</li>
					<li>Help and Support for non-bundled 3rd party plugins (i.e. plugins you install yourself later on)</li>
				</ul>
				<p><?php esc_html_e( 'More details about item support can be found in the ThemeForest' ,'foodchow'); ?> <a
					href="http://themeforest.net/page/item_support_policy" target="_blank"><?php esc_html_e( 'Item Support Policy' ,'foodchow'); ?></a>. </p>
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue' ,'foodchow'); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
						<?php
					}
		...
		public function envato_setup_ready() {
			...
			<h1><?php esc_html_e( 'Your Website is Ready!','foodchow' ); ?></h1>
			...
			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e( 'Next Steps','foodchow' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
							href="https://twitter.com/@webinane"
							target="_blank"><?php esc_html_e( 'Follow @webinane on Twitter','foodchow' ); ?></a>
						</li>
						<li class="setup-product"><a class="button button-next button-large"
							href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!' ,'foodchow'); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e( 'More Resources','foodchow' ); ?></h2>
					<ul>
						<li class="documentation"><a href="https://webinane.ticksy.com/"
							target="_blank"><?php esc_html_e( 'Read the Theme Documentation','foodchow' ); ?></a>
						</li>
						<li class="howto"><a href="https://wordpress.org/support/"
							target="_blank"><?php esc_html_e( 'Learn how to use WordPress','foodchow' ); ?></a>
						</li>
						<li class="rating"><a href="http://themeforest.net/downloads"
							target="_blank"><?php esc_html_e( 'Leave an Item Rating' ,'foodchow'); ?></a></li>
							<li class="support"><a href="https://webinane.ticksy.com/"
								target="_blank"><?php esc_html_e( 'Get Help and Support','foodchow' ); ?></a></li>
							</ul>
						</div>
					</div>
					<?php
				...
				// request the token from our bounce url.
					...
					// the first time this key is used, it is set and locked on the server.
						...
						if ( is_wp_error( $response ) ) {
							...
							$class         = 'error';
							echo "<div class=\"$class\"><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s','foodchow' ), $error_message ) . '</p></div>';
						} else {
					...
					add_settings_section(
						...
						sprintf( esc_html__( 'Login for %s updates','foodchow' ), $this->envato_username ),
			...
			// Items setting.
					add_settings_field(
						...
						esc_html__( 'oAuth Login','foodchow' ),
				...
				if ( !$option || empty($option['oauth']) || empty( $option['oauth'][ $this->envato_username ] ) || empty($envato_items) || empty($envato_items['items']) || !envato_market()->items()->themes( 'purchased' )) {
					...
					if ( ! $dissmissed_time || $dissmissed_time < strtotime('-7 days') ) {
						...
						<div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
							<p><?php
							_e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.','foodchow' );
							?></p>
							<p>
								<?php printf( __( '<a class="button button-primary" href="%s">Activate Updates</a>','foodchow' ),  esc_url($this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ); ?>
							</p>
						</div>
		...
		function revslider_export($dir = '') {
			...
			foreach ($sliders as $k => $s) {
				...
				foreach ($slides as $ss) {
					...
					if ($slider_id == foodchow_set($ss, 'slider_id'))
        ...
        function layerslider_export($dir = '') {
        	...
        	foreach ($sliders as $k => $s) {
                ...
                /*foreach ($slides as $ss) {
                    ...
                    if ($slider_id == foodchow_set($ss, 'id'))
                        $data['slider'][$k]['slides'][] = $ss;
                    }*/
                }
        ...
        function remaining_media($file = '') {
        	...
        	$stylefolder = basename(get_theme_mod('dtbwp_site_style',$this->get_default_theme_style()));
        	$theme_style = __DIR__ . '/content/'.$stylefolder.'/';
        	$file = $theme_style.'media.xml';
        	if (file_exists($file)) {
        		define('WP_LOAD_IMPORTERS', true);
        		if (!class_exists('WP_Import')) {
        			include_once( ABSPATH . 'wp-content/plugins/foodchow/wordpress-importer/wordpress-importer.php');
        		}
