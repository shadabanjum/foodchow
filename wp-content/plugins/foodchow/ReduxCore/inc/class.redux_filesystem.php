    ...
    if ( ! class_exists( 'Redux_Filesystem' ) ) {
        class Redux_Filesystem {
            ...
            public function __construct() {
                $this->parent->admin_notices[] = array(
                    'type'    => 'error',
                    'msg'     => '<strong>' . __( 'File Permission Issues', 'foodchow' ) . '</strong><br/>' . sprintf( __( 'We were unable to modify required files. Please check your permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%s" target="_blank">outlined here</a>.', 'foodchow' ), 'https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants' ),
                    'id'      => 'redux-wp-login',
                    'dismiss' => false,
                );
            }
            ...
            public function ftp_form() {
                if ( isset( $this->parent->ftp_form ) && ! empty( $this->parent->ftp_form ) ) {
                    ...
                    echo __( 'File Permission Issues', 'foodchow' );
		...
		require_once ABSPATH . '/wp-includes/pluggable.php';
			...
			if (empty($res) || $res == false || $res == '' ) {
                    ...
                    $this->parent->admin_notices[] = array(
                        'type'    => 'error',
                        'msg'     => __( 'File Permission Issues', 'foodchow' ),
                        'id'      => 'redux-wp-login',
                        'dismiss' => false,
                    );
