...
if (  ! class_exists(  'Foodchow_Resizer'  )  ) {
	class Foodchow_Resizer {
		public function Foodchow_resize( $url, $width = null, $height = null, $crop = true, $align = 'c', $image_url = false, $retina = false  ) {
			global $wpdb;
			$common = $this->Foodchow_info( $url, $width, $height, $crop, $align, $retina );
			...
			if( $image_url )
				...
				return '<img width="' . $width . '" height="' . $height . '" src="' . $resized_url . '" alt="notfound" class="wow fadeIn" data-wow-delay=".2s"/>';
		}

		public function Foodchow_info( $url, $width, $height, $crop, $align, $retina ) {
			if ( empty( $url ) ) {
        ...
        public function Foodchow_delResizer( $post_id ) {
        	...
        	$wp_upload_dir = wp_upload_dir(  );
        	$pathinfo = pathFoodchow_info( $metadata['file'] );
